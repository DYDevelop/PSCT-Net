...
from lib.utils.metrics_np import MAE, MSE, Peak_Signal_to_Noise_Rate, Structural_Similarity, Cosine_Similarity, \
  Peak_Signal_to_Noise_Rate_3D, lpips_3d
import copy
import tqdm
import torch
import numpy as np
...
def scoring(generate_CT_transpose, real_CT_transpose):
    # CT range 0-1
    ...
    lpips_score = lpips_3d(generate_CT_transpose, real_CT_transpose)
    
    # CT range 0-2500
    generate_CT_transpose = tensor_back_to_unMinMax(generate_CT_transpose, 0, 2500).astype(np.int32)
    real_CT_transpose = tensor_back_to_unMinMax(real_CT_transpose, 0, 2500).astype(np.int32)
    psnr_3d = Peak_Signal_to_Noise_Rate_3D(real_CT_transpose, generate_CT_transpose, size_average=False, PIXEL_MAX=2500)
    psnr = Peak_Signal_to_Noise_Rate(real_CT_transpose, generate_CT_transpose, size_average=False, PIXEL_MAX=2500)
    ssim = Structural_Similarity(real_CT_transpose, generate_CT_transpose, size_average=False, PIXEL_MAX=2500)
    ...
    # if np.isnan(cosinesimilarity).any() or (cosinesimilarity > 1).any():
    #     continue
...
def evaluate(args):
  ...
  for epoch_i, data in tqdm.tqdm(enumerate(dataloader)):
    ...
    metrics_list = scoring(generate_CT_transpose, real_CT_transpose)
