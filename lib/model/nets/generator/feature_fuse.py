...
def creategrid(z, y, x):
    tensor = tuple([torch.linspace(start=-1, end=1, steps=z),
                    torch.linspace(start=-1, end=1, steps=y),
                    torch.linspace(start=-1, end=1, steps=x)])
    
    grid = torch.stack(torch.meshgrid(*tensor, indexing='ij'), dim=-1)
...
class BasicBlock(nn.Module): # except last layer, each block
    ...
    def forward(self, x):
        ...
        identity = x
        dx = self.fc(x)
        dx = torch.sin(dx) # torch.Size([2097152, 64]) 128*128*128, 64
        dx = self.fc(dx)
        out = dx + identity
        out = torch.sin(out)
...
class Resnet(nn.Module):
    ...
    def __init__(self, hidden_dim, num_blocks, view_direction):
        ...
        self.fc = SirenLinear1(hidden_dim+9, hidden_dim, bias=True, is_first=True, omega_0=50)
        self.blocks = nn.ModuleList([BasicBlock(hidden_dim) for i in range(num_blocks)])
        self.conv_out = nn.Conv2d(50, 32, 3, 1, 1)
    ...
    def forward(self, w, coords):
        ...
        batch, n_w_feat, H, W = w.shape  # batch size, feature number, image downsize area, such as B, 50, 32, 32
        ...
        if self.view_direction == 'frontal':
            w = w.unsqueeze(3).repeat(1, 1, 1, H, 1) # torch.Size([4, 704, 4, 4, 4])

        else:
            w = w.unsqueeze(4).repeat(1, 1, 1, 1, H) # torch.Size([4, 704, 4, 4, 4])
        ...
        w = w.permute(0, 2, 3, 4, 1).reshape(-1, n_w_feat) # (4*4*4, 704) torch.Size([256, 704])

        coords = encode_position(coords)  # (B*4*4*4, 9) / 256, 9

        device = w.device  # w의 디바이스를 기준으로 설정
        coords = coords.to(device)  # coords를 동일한 디바이스로 이동

        x = torch.cat((w, coords), dim=1)  # 704 + 9 = 41

        x = self.fc(x)

        x = torch.sin(x) # torch.Size([1, 2097152, 32]) 1, 32*32*32, 32
