...
import torch
import numpy as np
from lib.model.nets.generator.back_pro import Backprojector_single
...
class Dimension_UpsampleBPBlock(nn.Module):
  def __init__(self, input_channel, output_channel, norm_layer2d, norm_layer3d, view, activation=nn.ReLU(True), use_bias=True):
    super(Dimension_UpsampleBPBlock, self).__init__()
  ...
  def forward(self, input):
    # input's shape is [NCHW]
    N,_,H,W = input.size()
    bp_block = Backprojector_single(interp="nearest", input_hw=H, output_hw=W)
    # expand to [NCDHW]
    return self.conv_block(bp_block(self.compress_block(input), self.view))
