...
from .loss.multi_gan_loss import *
...
class CTGAN(Base_Model):
  ...
  def init_network(self, opt):
    ...
    # map lpips loss
    if self.opt.map_projection_lpips_lambda > 0:
      self.loss_names += ['map_m_lpips']

    # 3d lpips loss
    if self.opt.lpips_3d_lambda > 0:
      self.loss_names += ['lpips_3d']

    # 3d net lpips loss
    if self.opt.demo_lambda > 0:
      self.loss_names += ['demo']
  ...
  def init_loss(self, opt):
    ...
    # map lpips loss
    self.criterionMap_lpips = LpipLoss().to(self.device)

    self.lpips_3d = LpipLoss_3D().to(self.device)
    self.demo = Demo_loss().to(self.device)
  ...
  def get_projection2d_loss(self, model_output, target):

      l1_loss = torch.nn.L1Loss().cuda()
      diff = 0

      for i in [1, 2, 3]:
          target_slice = self.output_map(target, i + 1)
          model_output_slice = self.output_map(model_output, i + 1)
          diff += l1_loss(target_slice, model_output_slice)

      diff = diff / 3

      return diff

  def get_wavelet2d_loss(self, model_output, target):

      wav_loss = WaveletLoss2D().cuda()
      diff = 0

      for i in [1, 2, 3]:
          target_slice = self.output_map(target, i + 1)
          model_output_slice = self.output_map(model_output, i + 1)
          diff += wav_loss(target_slice, model_output_slice)

      diff = diff / 3

      return diff

  def get_wavelet3d_loss(self, model_output, target):

      wav_loss = WaveletLoss3D().cuda()

      return wav_loss(target, model_output)

  def get_perceptual3d_loss(self, model_output, target):

      per_loss = LpipLoss_3DNet().cuda()

      return per_loss(target, model_output)

  def get_ssim3d_loss(self, model_output, target):

      ssim_loss = SSIM3D(window_size=11).cuda()

      return 1.0 - ssim_loss(target, model_output)
  ...
  def backward_G(self):
    ...
    map_m_lpips_lambda = self.opt.map_projection_lpips_lambda
    lpips_3d_lambda = self.opt.lpips_3d_lambda
    demo_lambda = self.opt.demo_lambda
    ...
    # map lpips loss
    if map_m_lpips_lambda > 0:
      self.loss_map_m_lpips = 0.
      for direction in self.multi_view:
        self.loss_map_m_lpips += self.criterionMap_lpips(
          getattr(self, 'G_Map_{}_fake'.format(direction)),
          getattr(self, 'G_Map_{}_real'.format(direction))) * map_m_lpips_lambda
      self.loss_map_m_lpips = self.loss_map_m_lpips / len(self.multi_view)

    if lpips_3d_lambda > 0:
      self.loss_lpips_3d = self.lpips_3d(self.G_fake_D, self.G_real_D) * lpips_3d_lambda
    
    if demo_lambda > 0:
      self.loss_demo = self.demo(self.G_fake_D, self.G_real_D) * demo_lambda
    ...
    if map_m_lpips_lambda > 0:
      self.loss_total_G += self.loss_map_m_lpips
    if lpips_3d_lambda > 0:
      self.loss_total_G += self.loss_lpips_3d
    if demo_lambda > 0:
      self.loss_total_G += self.loss_demo
    if self.auxiliary_loss:
